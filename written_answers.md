...
**Q4 - What Is Broken**
The most significant flaw is that the embedding model (all-MiniLM-L6-v2) is general-purpose, not fine-tuned for ClearPath's domain vocabulary. Project management terms like "sprint velocity," "burndown chart," and "custom workflow states" don't get strong semantic matches. A user asking about "velocity tracking" can fail to retrieve a chunk that says "track your team's sprint velocity" because the model doesn't strongly connect those phrases. I shipped it anyway because fine-tuning requires labeled (query, relevant_chunk) pairs that I don't have, and the general model performs acceptably on roughly 80% of support questions. The fix I'd implement is hybrid retrieval: BM25 keyword search combined with semantic search. BM25 catches exact term matches that embeddings miss, and the semantic side still handles paraphrases. This is a standard RAG improvement that doesn't require labeled data and would immediately reduce misses for domain-specific terminology.
